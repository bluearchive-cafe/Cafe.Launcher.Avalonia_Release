...
1. 首次启动后，在设置中选择界面语言
2. 确认游戏安装路径（默认：启动器目录下 `YostarGames\BlueArchive_JP`）
3. 点击主界面 **安装** 按钮，开始下载游戏
4. 安装完成后，点击 **启动** 进入游戏
5. 游戏有更新时，按钮自动变为 **更新**
...
| 下载与网络 | 代理（直连 / 系统代理）、下载源（官方 CDN / Cafe CDN）、速度限制、更新通道（stable / beta）、日志级别 |
| 外观 | 主题（浅色 / 深色 / 跟随系统）、主题色（默认 / 系统 / 壁纸 / 自定义）、背景（内置 / 远程 / 自定义）、Toast 通知、远程内容卡片 |
| 关于 | 版本信息、检查更新、官方网站、GitHub、查看日志、导出日志、打开数据目录、免责声明 |
